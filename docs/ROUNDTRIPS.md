...
| `gc` op (#175) | lease GET+PUT + 1 LIST of `wal/_superseded/` + per object 1 HEAD + 1 conditional DELETE + 2 manifest CAS + lease GET+DELETE | refs-level (never reads pack data); ≤ `GC_MAX_PACKS_PER_UNIT`=32 packs per unit, so a pass is bounded. Markers live in their own prefix: the LIST walks markers, not every pack/idx/side-file |
